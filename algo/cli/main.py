...
def backtest_bhavcopy(
    ...
    # `kill_switch` is unconditionally built above (both branches of the
    # `if config is not None` set `settings`, and `KillSwitch(...)` follows
    # unconditionally) - unlike the `backtest` command this mirrors, it can
    # never be None here, so a `kill_switch or KillSwitch(...)` fallback was
    # dead code left over from that copy.
    store: StateStore | None = None
    if state is not None:
        store = StateStore(state)
